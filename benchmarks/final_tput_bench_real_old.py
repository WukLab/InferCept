...
class APIExecutor:
    ...
    def get_new_sampling_params(self, request_id: str) -> Tuple[SamplingParams, int]:
        ...
        if experiment_num >= len(experiments):
            experiment = {"prompt_size": 0, "api_exec_time": 0, 'api_ret_len': 0}
        else:
            experiment = experiments[experiment_num]
        prompt_size, api_exec_time, api_return_len = experiment["prompt_size"], experiment["api_exec_time"], experiment["api_ret_len"]
...
def run_vllm(
    ...
    for request_id, experiments in exp_json.items():
        ...
        for i, experiment in enumerate(experiments):    
            if "prompt" in experiment and "prompt_size" not in experiment:
                ...
                if i == 0 and experiment['prompt_size'] >= 2048:
                    print(f'long prompt: {new_2_old_key[int(request_id)]}')
            if i == 0:
                prompt_lens.append(experiment['prompt_size'])
            if "api_output" in experiment and "api_ret_len" not in experiment:
                experiment["api_ret_len"] = len(engine.tokenizer.encode(experiment["api_output"]))
                if experiment["api_ret_len"] >= 2048:
                    print(f'long ret: {new_2_old_key[int(request_id)]}')
            if i > 0:
                ts += experiment["prompt_size"] + experiment["api_ret_len"]
                ret_lens.append(experiment['api_ret_len'])
            else:
                ts += experiment["api_ret_len"]
                ret_lens.append(experiment['api_ret_len'])
            if "api_exec_time" in experiment:
                api_exec_times.append(experiment['api_exec_time'])
            api_engine.total_output_toks[request_id] = ts
        output_lens.append(min(ts, 2048 - prompt_lens[-1]))
        num_api_calls.append(len(experiments))
    ...
    with open(f'./{args.msg}/all_trace.csv', 'w+') as t:
        for r in requests:
            ...
            latencies.append(final_norm_latency)
            t.write(f'{r.request_id}, {r.finished}, {r.end_time-r.start_time}, {api_time}, {start_length}, {end_length}, {final_norm_latency}\n')
