...
from vllm.model_executor.layers.simulator import Simulator
...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        # partial forwarded seqs will be peeled out
        hidden_states = _prune_hidden_states(hidden_states, input_metadata)
        # torch.save(hidden_states, f"ts/hs_c_{Sampler.i_hs}.pt")
        # Sampler.i_hs += 1
        ...
        if _USE_SIMULATOR:
            return self.simulator(input_metadata)
...
# NOTE: this will also reorder the output logits: group by sampling type
#       seqs whose forwarded token is not logical query will be ignored
def _prune_hidden_states(
    ...
    start_idx = 0
    running_query_idx = 0 # prompt seqs are not included in the running len list
    for i, seq_group in enumerate(input_metadata.seq_groups):
        ...
        if i < input_metadata.num_prompts:
            ...
            # if input_metadata.is_generating_new_token[i]:
            last_token_indices[sampling_type].append(start_idx + prompt_len - 1)
            start_idx += prompt_len
        else:
            for _ in seq_ids:
                next_start = start_idx + input_metadata.running_query_lens[running_query_idx]
                # if input_metadata.is_generating_new_token[input_metadata.num_prompts + running_query_idx]:
                last_token_indices[sampling_type].append(next_start - 1)
